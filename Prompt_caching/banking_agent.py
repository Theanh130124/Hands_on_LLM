...
from crewai import Agent, Task, Crew, LLM  #Xây dựng crewAi để xây dựng Agent
...
class BankingAgent:
    ...
    def explain(self, input_question: str) -> tuple[str, dict]:
        ...
        hit = retrieve_from_cache(input_question)  #Nhớ check caching trước
        if hit:
            return hit["response"], {
                "cached": True,
        ...
        outcome = self.crew.kickoff(inputs={"question": input_question}) #Nếu không có hỏi về LLM
